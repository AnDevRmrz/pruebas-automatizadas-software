...
exports.CreatePagePage = class CreatePagePage {
  constructor(scenario) {
    ...
    this.selectors = {
      ...
      backButton: 'a[data-test-link="pages"]',
  ...
  async goBack() {
    try {
      let statusText = '';
      do {
        statusText = await this.scenario.getPage().locator(this.selectors.savingStatusText).textContent();
        await this.waitForLoad();
      } while (!statusText.includes('Draft'));

      await this.scenario.getPage().locator(this.selectors.backButton).click();
      await this.waitForLoad();
      if (await this.scenario.getPage().locator(this.selectors.leaveButton).isVisible()){
  ...
  async fillUrlButton(url) {
    await this.scenario
      .getPage()
      .locator(this.selectors.buttonInputUrl)
      .fill(url);
    await this.scenario.screenshot();
  }
  ...
  async fillButton(text, url) {
    await this.scenario
      .getPage()
      .locator(this.selectors.markdownButton)
      .click();
    await this.scenario.screenshot();
    await this.scenario
      .getPage()
      .locator(this.selectors.buttonMarkdownPageButton)
      .click();
    await this.scenario.screenshot();
    await this.scenario
      .getPage()
      .locator(this.selectors.buttonInputText)
      .fill(text);
    await this.scenario.screenshot();
  ...
  async publishPage() {
    ...
    await this.scenario
      .getPage()
      .locator(this.selectors.finalReviewButton)
      .click();
    await this.waitForLoad();
    await this.scenario
      .getPage()
      .locator(this.selectors.confirmPublishButton)
      .click({ force: true });
    await this.scenario.screenshot();
  }
  ...
  async closePublishFlow() {
    await this.scenario
      .getPage()
      .locator(this.selectors.closePublishButton)
      .click();
    await this.scenario.screenshot();
  }
  ...
  async verifyTitleInModal(title) {
    ...
    const modalTitle = await this.scenario
      .getPage()
      .locator(this.selectors.modalTitle)
      .innerText();
    await this.scenario.screenshot();
  ...
  async verifyDescriptionInModal(description) {
    const modalDescription = await this.scenario
      .getPage()
      .locator(this.selectors.postExcerpt)
      .innerText();
    await this.scenario.screenshot();
    return modalDescription === description;
  }
  async getErrorMessage() {
    const errorElement = await this.scenario
      .getPage()
