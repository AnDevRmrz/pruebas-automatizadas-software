...
describe( `${appName} under monkeys`, function() {
    ...
    it(`visits ${appName} and survives monkeys`, function() {
        ...
        if(pcg === 100){
            ...
            cy.get("#blog-title").then(inputTitle => {

                cy.wrap(inputTitle).type("Monkey Test Title");
            });

            cy.get("#name").then(inputName => {

                cy.wrap(inputName).type("Monkey Test Name");
            });

            cy.get("#email").then(inputEmail => {

                cy.wrap(inputEmail).type("equipo3@monkeytest.com");
            });

            cy.get("#password").then(inputPassword => {

                cy.wrap(inputPassword).type("ihEHubpNSTpUQS4#123");
            });

            cy.get("#ember4").then(createAccountButton => {

                cy.wrap(createAccountButton).click();
            });
