...
Then('I can see a row with tag title name {string}', async function (tagName) {    
    ...
    let titleElements = await this.driver.$$('.gh-list-row .gh-tag-list-name');
...
Then('I can see a row with tag description {string}', async function (tagDescription) {
    ...
    let descriptionElements = await this.driver.$$('.gh-list-row .gh-tag-list-description');
...
Then('I can see a row with slug {string}', async function (slug) {    
    let slugElements = await this.driver.$$(`.gh-list-row span[title='${slug}']`);    
    assert.equal(slugElements.length, 1);    
});
...
Then('I cannot see a row with slug {string}', async function (slug) {    
    let slugElements = await this.driver.$$(`.gh-list-row span[title='${slug}']`);    
    assert.equal(slugElements.length, 0);
});